...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createBookingRow(booking) {
        ...
        // Confirm Trip
        const confirmTripCell = document.createElement('td');
        const truncatedConfirmTrip = booking.confirmTrip ? booking.confirmTrip.substring(0, 30) + (booking.confirmTrip.length > 30 ? '...' : '') : '';
        confirmTripCell.innerHTML = `
            <div style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${Config.utils.escapeHtml(booking.confirmTrip || 'No trip name')}">
                ${Config.utils.escapeHtml(truncatedConfirmTrip)}
            </div>
        `;
 ...
 `;
        ...
        row.appendChild(confirmTripCell);
